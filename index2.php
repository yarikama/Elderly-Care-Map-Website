...
<div style="width: 300px; padding: 20px; box-shadow: 0px 0px 10px 0px rgba(0,0,0,0.1); border-radius: 8px; position: absolute; right: 40px; top: 50px; z-index: 100; background-color: white;">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <button id="searchBtn" style="background-color: #008CBA; color: white; padding: 12px 24px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px;">搜索</button>
    </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // 從資料庫獲取縣市列表並添加到選單
        fetch(baseURL + '/getCounties').then(response => response.json()).then(data => {
            data.forEach(county => {
        ...
        countySelect.addEventListener('change', function() {
            ...
            fetch(baseURL + '/getDistricts/' + selectedCounty).then(response => response.json()).then(data => {
